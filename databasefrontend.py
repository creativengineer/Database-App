...
def delete_command():
    databasebackend.delete(selected_row[0])

def view_command():
    list.delete(0,END)
    for row in databasebackend.view():
        list.insert(END,row)

def search_command():
    list.delete(0,END)
    for row in databasebackend.search(date_text.get(),earning_text.get(),exercise_text.get(),study_text.get(),diet_text.get(),python_text.get()):
        list.insert(END,row)

def add_command():
    databasebackend.insert(date_text.get(),earning_text.get(),exercise_text.get(),study_text.get(),diet_text.get(),python_text.get())
